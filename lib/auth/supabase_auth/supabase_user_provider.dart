...
class MondahaSupabaseUser extends BaseAuthUser {
  MondahaSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> mondahaSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = MondahaSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
